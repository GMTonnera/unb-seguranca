...
class AES:
    def __init__(self):
        ...
        self.ctrQueue = []
        self.ctrQueueLock = threading.Lock()
    ...
    def manageFile(self, filename, flag):
        filePath = Path(filename)
        extesion = filePath.suffix
        data = []
        newFilename = ""
        
        if extesion == ".txt":
            if flag:
                newFilename = filename[:filename.index('.')] + '-AESencrypted.txt'
                data = self.readTextFile(filename)
            else:
                newFilename = filename[:filename.index('.')] + '-AESdecrypted.txt'
                data = self.readTextFile(filename)
        else:
            data = self.imageToBytes(filename)
            if flag:
                newFilename = filename[:filename.index('.')] + '-encrypted.' + extesion
            else:
                newFilename = filename[:filename.index('.')] + '-decrypted.' + extesion
    ...
    def middleRound(self, state, roundKey):
        ...
        # ShiftRows
        newState = newState[0:4] + newState[5:8] + int.to_bytes(newState[4]) + newState[10:12] + newState[8:10] + int.to_bytes(newState[15]) + newState[12:15]
        ...
        newState = int.to_bytes(newColumns[0]) + int.to_bytes(newColumns[4]) + int.to_bytes(newColumns[8]) +  int.to_bytes(newColumns[12]) + int.to_bytes(newColumns[1]) + int.to_bytes(newColumns[5]) + int.to_bytes(newColumns[9]) + int.to_bytes(newColumns[13]) + int.to_bytes(newColumns[2]) + int.to_bytes(newColumns[6]) + int.to_bytes(newColumns[10]) + int.to_bytes(newColumns[14]) + int.to_bytes(newColumns[3]) + int.to_bytes(newColumns[7]) + int.to_bytes(newColumns[11]) + int.to_bytes(newColumns[15]) 
    ...
    def threadEncryptCTRMode(self, data, encryptedCounters, id):
        encryptedData = b""
    ...
    def encryptCTRMode(self, filename, key, iv, rounds=10):
        encryptedData = b""
        self.encryptedCounters = b""
        self.ctrQueue = []
        # Extrair bytes do arquivo
        data = self.readTextFile(filename)
        newFilename = filename[:filename.index('.')] + '-AES_CTR_encrypted.txt'
        
        # Realizar a expansao da chave
        expandedKey = self.keyExpansion(key, rounds)
        
        # Calcular contadores
        counters = self.genCounters(iv, len(data) // 16+1)
        ...
        t1 = threading.Thread(target=self.threadEncryptCTRMode, args=[data[:(len(data)//16 + 1) // 2 * 16], self.encryptedCounters[:(len(data)//16 + 1) // 2 * 16], 1])
        t1.start()
        
        # Thread 2 -> criptografa a segunda metade dos dados
        t2 = threading.Thread(target=self.threadEncryptCTRMode, args=[data[(len(data)//16 + 1) // 2 * 16:], self.encryptedCounters[(len(data)//16 + 1) // 2 * 16:], 2])
        t2.start()
        ...
        if self.ctrQueue[0][0] == 1:
            encryptedData = self.ctrQueue[0][1] + self.ctrQueue[1][1]
        else:
            encryptedData = self.ctrQueue[1][1] + self.ctrQueue[0][1]
        
        self.genEncryptedFile(newFilename, encryptedData)
                
    
    def decryptCTRMode(self, filename, key, iv, rounds=10):
        decryptedData = b""
        self.encryptedCounters = b""
        self.ctrQueue = []
        # Extrair bytes do arquivo
